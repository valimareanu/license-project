...
$movie_date = $core->db->query("SELECT movie_date FROM filme WHERE id=$id_sent")->rows[0];
if(strtotime(date('d-m-Y')) >= strtotime($movie_date['movie_date'])) {
...
if($m_date == true && !$q)mysql_query("INSERT INTO ratings (id,add_date) values ($id_sent,curdate())");
...
if(!$voted && $m_date == true) {     //if the user hasn't yet voted, then vote normally...
...
elseif($m_date == false) {$new_back[] .= '<span class="invalid">You can\'t vote yet. Movie wasn\'t launched.</span></p></div>';}
